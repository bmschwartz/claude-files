...
| `/git-review` | Fresh `Explore`/`general-purpose` subagent with adversarial prompt, diff, and SPEC.md. Write to `.claude/reviews/<sanitized-branch>/<YYYYMMDD-HHMMSS>/REVIEW_SUMMARY.md` (standard round directory — same structure as `/git-review` output). Tag `_review-prompt.md` with `source: fallback` metadata. Update REVIEW.md. Present findings interactively (Apply/Skip). Convergence loop still applies. |
